  ...
  return (
    ...
    <div className="container ">
      <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 " >
        {goodHeroes.length ?
          goodHeroes.map(card => (
            <div className="mb-5" key={card.id}  ><Card title={card.name} id={card.id} url={card.image.url} gob={card.biography.alignment} powerstats={card.powerstats} heroLG={goodHeroes} /></div>
          ))
          :
